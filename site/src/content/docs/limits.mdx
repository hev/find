---
title: "Limits"
description: "The hard boundaries of hev find: corpus scope, recall ceiling, single-call knowledge-graph build, frontmatter parsing, latency, and adapter requirements — what it deliberately does not do."
group: "Overview"
order: 4
---
...
These are the boundaries you should know before adopting hev find. None of them
are bugs — they're the edges of what the current design covers.

## The corpus is your content collection

hev find searches the Astro content collection(s) you configure, and nothing
...
Deep links are only correct as long as the heading slugs hev find generates
match Astro's rendered `id` attributes. hev find uses the same `github-slugger`
to stay aligned, and ships a `hev-find-kg verify` command that fails if any
...
The `/api/find` endpoint renders on demand, so a **fully static build cannot
...
runs `/api/find`. The key is never exposed to the browser. If the key isn't
