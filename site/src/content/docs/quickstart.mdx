...
hosted endpoint — no Astro required. See [Frameworks](/docs/frameworks) for the
step-by-step on yours.
</Callout>
...
<Steps items={[
  ...
  { title: "Build the digest", body: "Use the Claude Code skill (no key) and commit the .hev-ask/ tree." },
]} />
...
Both paths are **incremental and hash-gated** — model work only for the sections
that changed — and the integration runs the build automatically during
`astro build` when a key is present. See the [CLI reference](/docs/api/cli).
