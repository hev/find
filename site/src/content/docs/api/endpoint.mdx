...
`GET /api/ask` returns the digest's baked-in suggestions and the loop model,
with no query and no model call:
...
suggested questions. An empty `suggestions` array (a digest without them, or no
digest at all) just means the overlay shows none.
...
The endpoint reads the key named by the configured provider —
`ANTHROPIC_API_KEY` by default, `OPENAI_API_KEY` or `OPENROUTER_API_KEY` when
the [`provider` option](/docs/api/configuration) is set — from, in order: the
adapter runtime env (`locals.runtime.env`, e.g. Cloudflare), `process.env`,
then `import.meta.env`. Set it wherever your host injects server secrets; it is
...
controls how much prompt and answer text ships with each event. Without a key
it's a no-op; the answer path never depends on telemetry.
